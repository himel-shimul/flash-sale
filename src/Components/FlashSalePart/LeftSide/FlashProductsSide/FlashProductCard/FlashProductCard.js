...
const FlashProductCard = ({ product }) => {
  ...
  return (
    <div className="border w-screen group relative m-2 p-1 lg:w-[100%] mx-3">
      <p className="text-sm rounded-sm text-red-600 bg-red-200 inline p-1 ">{type}</p>
      <img className="lg:w-[280px]  hover:-scale-x-100" src={img} alt="" />
      <p className="text-xs text-gray-500 my-2">{category}</p>
      <p className="text-base font-semibold lg:w-5/6 my-2">{name}</p>
